...
UAbilityTask_SpawnGameplayActor* UAbilityTask_SpawnGameplayActor::SpawnGameplayActor(UGameplayAbility* OwningAbility, FGameplayAbilityTargetDataHandle TargetData, TSubclassOf<AActor> InClass, AActor* Owner)
{
...
bool UAbilityTask_SpawnGameplayActor::BeginSpawningActor(UGameplayAbility* OwningAbility, FGameplayAbilityTargetDataHandle TargetData, TSubclassOf<AActor> InClass, AActor*& SpawnedActor)
{
	if (Ability && Ability->GetCurrentActorInfo()->IsNetAuthority() && ShouldBroadcastAbilityTaskDelegates())
	{
		...
		if (World)
		{
			SpawnedActor = World->SpawnActorDeferred<AActor>(InClass, FTransform::Identity, CachedOwner, NULL, ESpawnActorCollisionHandlingMethod::AlwaysSpawn);
